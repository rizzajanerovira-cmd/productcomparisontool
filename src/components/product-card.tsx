...
export function ProductCard({
  ...
  return (
    <Card className="overflow-hidden rounded-[1.75rem] border border-black/5 bg-white/90 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.35)]">
      ...
      <CardHeader className="space-y-3 px-4">
        ...
        <div className="space-y-2">
          <CardTitle className="min-h-[3.5rem] text-lg font-semibold tracking-tight text-slate-950">
            {product.name}
          </CardTitle>
          <p className="min-h-[3.75rem] text-sm leading-6 text-slate-600">
            {product.summary}
          </p>
        </div>
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Estimated retail price</p>
          </div>
      ...
      <CardFooter className="flex flex-col gap-3 border-t border-black/5 bg-slate-50/70 px-4 py-4 sm:flex-row">
        <Button
        ...
        <Button
          variant="outline"
          className="w-full rounded-full border-slate-200 bg-white text-slate-700"
...
function SpecPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-black/5 bg-slate-50/90 p-3">
